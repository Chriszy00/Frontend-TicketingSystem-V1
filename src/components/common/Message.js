				...
				<div class="order">
					<div className='row' style={{ height: '62vh' }}>
						...
						<div className='bg-primary col-3'>
							...
							<div key={index} onClick={() => setSelectedUserId(user.id)} style={{ cursor: 'pointer' }}>
						...
						{roleName === 'ROLE_USER' ? (
						<div ref={messagesContainerRef} style={{ width: '100%', maxHeight: '58vh', overflowY: 'auto' }}>
						<h5>Messages</h5>
						{receivedMessages.map((message, index) => (
							<div
							key={index}
							className={`px-3 py-1 mb-1 rounded-5 text-white ${message.sender.id == localStorage.getItem('userId') ? 'bg-primary ms-auto' : 'bg-secondary'}`}
						...
						{/* If type is note */}
							...
							{receivedMessages.map((message, index) => (
								<div 
								key={index}>
									{message.sender && message.sender.id !== parseInt(selectedOption, 10) && (
									<div style={{fontSize:'12px', width: 'fit-content' }}
									className={`ms-1 text-black ${
										message.type === 'reply'
												? 'me-1 ms-auto'
												: message.type === 'note'
													? 'me-1 ms-auto' : 'ms-1 text-end'
									}`}
									>{message.sender.firstName} {message.sender.lastName}</div>
								)}
								<div
								className={`px-3 py-1 mb-1 rounded-5 text-white ${
									message.sender && message.sender.id === selectedOption
										? 'bg-secondary'
										: message.type === 'reply'
											? 'bg-primary ms-auto'
											: message.type === 'note'
												? 'bg-warning ms-auto' : 'bg-secondary'
								}`}
								style={{ width: 'fit-content' }}
							>
								{message.message}
					...
					<div style={{width:'100%'}}>
						...
						style={{ height: '5vh', width: '94%', border: '1px solid #ced4da' }}
						className='border rounded-3'
						placeholder="Type your message..."
						/>
						<button onClick={handleSendMessage} className='ms-3 btn btn-outline-primary'>Send</button>
					</div>
					) : roleName === 'ROLE_INTERNAL' ? (
					<div style={{width:'100%'}}>
						<select value={selectedOption} onChange={handleOptionChange} className='border rounded-3 py-2'>
						...
						placeholder={`Type your ${selectedOption === 'reply' ? 'reply' : 'note'}...`}
						/>
						<button onClick={handleSend} className='ms-3 btn btn-outline-primary'>Send</button>
					</div>
					) : null}
					</div>
				</div>
